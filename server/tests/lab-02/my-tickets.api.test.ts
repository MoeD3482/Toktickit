...
describe("GET /api/v1/tickets", () => {
  ...
  it("returns only Tickets owned by the authenticated Requester", async () => {
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "Requester A cannot connect to the campus VPN.",
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "Requester B cannot print from the office printer.",
  ...
  it("searches the authenticated Requester's Tickets", async () => {
    const prisma = getPrisma();
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "The requester cannot connect to the campus VPN.",
        requestedPriority: "High",
        status: "New",
  ...
  it("filters the authenticated Requester's Tickets", async () => {
    const prisma = getPrisma();
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "This Ticket should match the requested filters.",
        requestedPriority: "High",
        status: "New",
        clientRequestId: requestId1,
      },
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "This Ticket should not match the requested filters.",
    ...
    expect(
      res.body.data.some(
        (ticket: { summary: string }) =>
          ticket.summary ===
          "Low priority other ticket"
      )
    ).toBe(false);
  });

  it("sorts the authenticated Requester's Tickets by Ticket Number", async () => {
    const prisma = getPrisma();
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "First Ticket used for sorting verification.",
        requestedPriority: "Low",
        status: "New",
        clientRequestId: requestId1,
        updatedAt: new Date(
    ...
    await prisma.ticket.create({
      data: {
        ...
        description:
          "Second Ticket used for sorting verification.",
        requestedPriority: "High",
        status: "New",
        clientRequestId: requestId2,
        updatedAt: new Date(
    ...
    const res = await agent.get(
      `/api/v1/tickets?search=${sortToken}` +
        `&sort=ticketNo&order=asc`
    );
    ...
    expect(
      res.body.data[0].ticketNo
    ).toBe(`TEST-SORT-A-${sortToken}`);

    expect(
      res.body.data[1].ticketNo
    ).toBe(`TEST-SORT-B-${sortToken}`);
  });

  it("paginates the authenticated Requester's Tickets and returns metadata", async () => {
    const prisma = getPrisma();
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      await prisma.ticket.create({
        data: {
          ticketNo:
            `TEST-PAGE-${pageToken}-${String(i).padStart(2, "0")}`,
          requesterId: requester!.id,
          requesterUserId: requester!.id,
          categoryId: category!.id,
          relatedSystemId: relatedSystem!.id,
          summary:
            `Pagination ${pageToken} ticket ${i}`,
          description:
            `Pagination test Ticket number ${i}.`,
          requestedPriority: "Medium",
          status: "New",
          clientRequestId,
        },
